    ...
    createNodeModuleBindings() {
        ...
        const nsPackage = m => {
            ...
            if (isNamespacedPackage) {
                ...
                return pkgs
                    .map(p => this.binder.makeMount(paths_1.localPath(`node_modules/${m}/${p}`), `scriptcraft-plugins/${m}/${p}`))
                    .join(' ');
            }
            return this.binder.makeMount(paths_1.localPath(`node_modules/${m}`), `scriptcraft-plugins/${m}`);
        };
        if (modules.length > 0) {
